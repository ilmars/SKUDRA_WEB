...
# from configapp.models import LoginFailures
...
class OAuth2Validator(RequestValidator):
    ...
    def validate_user(self, username, password, client, request, *args, **kwargs):
        ...
        #LDAP
        # from ldap import CustomLDAPBackend
        # CustomLDAPBackend.authenticate_ldap_user
        # check if user  has reached failure limit
        print(username)
        try:
            userObj = UserModel.objects.get(username=username)
            # objToCheck, createdTocheck = LoginFailures.objects.get_or_create(
            #             user=userObj,
            #             defaults={'user': userObj, 'count':0},
            #         )
            # if objToCheck.count >= 3:
            #     return False
        ...
        if u is not None and u.is_active:
            # obj, created = LoginFailures.objects.get_or_create(
            #             user=u
            #         )
            obj.count = 0
            obj.client = client.client_id
            # print('__________', type(request), request)
            # if request.headers.has_key('REMOTE_ADDR'):
            #     print('324324')
            obj.ip = request.headers['REMOTE_ADDR'] if 'REMOTE_ADDR' in request.headers else '-'
            obj.agent = request.headers['HTTP_USER_AGENT'] if 'HTTP_USER_AGENT' in request.headers else '-'
            print('obj', obj)
            obj.save()
            request.user = u
            return True
            ...
            print(".......", userObj, u)
            if userObj:
                obj, created = LoginFailures.objects.get_or_create(user=userObj)
                # print(obj, created)
                obj.count = F('count') + 1
                obj.client = client.client_id
                if 'REMOTE_ADDR' in request.headers:
                    ...
                    obj.ip = '-'
                if 'HTTP_USER_AGENT' in request.headers: 
                    obj.agent = request.headers['HTTP_USER_AGENT']
                else: 
                    obj.agent = '-'
                obj.save()
        return False
